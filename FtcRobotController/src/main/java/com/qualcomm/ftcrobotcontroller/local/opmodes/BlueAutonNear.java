 ...
 */
public class BlueAutonNear extends LinearOpMode {
    // motor declarations
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            switch (counter) {
                case 0:
                    if(!hasBeenSet) {
                        motorTargetsDrive = setDriveTarget(30.0d);
                        hasBeenSet = true;
                        clock.reset();
                    }
                ...
                case 3:
                    if(!hasBeenSet) {
                        motorTargetsTurn = setTurnTarget(145.0d);
                        hasBeenSet = true;
                        clock.reset();
                    }
                    finished = turnRight();
                    if(finished  || isPastTime(2.5d)) {
                ...
                case 9:
                    if(!hasBeenSet) {
                        if(tempMotorPosR - M_driveFR.getCurrentPosition() < 200) {
                            motorTargetsDrive = setDriveTarget(20);
                        } else {
                            motorTargetsDrive = setDriveTarget(24);
                        }
                        hasBeenSet = true;
                        clock.reset();
                    }
